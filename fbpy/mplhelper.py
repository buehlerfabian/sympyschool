...
def set_usetex(enable):
    ...
    if enable:
        plt.rcParams.update({"text.usetex":True,
                     "font.family":"serif",
                     "font.size":11,
                    "text.latex.preamble":r"\usepackage{mathpazo}\usepackage[locale=DE,per-mode=fraction,separate-uncertainty=true]{siunitx}"})
    else:
...
def sbplot(x, y, fig=None, ax=None, xlabel='x', ylabel='y', **plt_kwargs):
    """Wrapper around pyplot.plot with settings to produce schoolbook like axes.
    ...
    with plt.rc_context(rc):
        ...
        xlabel_offset_transform = ax.get_yaxis_transform() + ScaledTranslation(0,-2/72,fig.dpi_scale_trans)
        ylabel_offset_transform = ax.get_xaxis_transform() + ScaledTranslation(5/72,-5/72,fig.dpi_scale_trans)
...
def export_png(name, fig=None, scalefactor=1, transparent=False):
    ...
    Args:
        ...
        transparent (bool, optional): use transparent background. Defaults to False.
    """    
    if fig is None:
        fig = plt.gcf()
...
def set_minor_xticks(num_parts=2, ax=None):
    ...
    Args:
        num_parts (int, optional): number of subintervals between major ticks. Defaults to 2.
        ax (axes, optional): axes to use. Defaults to current axes.
    """    
    if ax is None:
        ax = plt.gca()
